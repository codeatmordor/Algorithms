...
class AllTaskSchedulingOrders {
  public static void printOrders(int tasks, int[][] prerequisites) {
    ...
    HashMap<Integer, Integer> inDegree = new HashMap<>(); // count of incoming edges for every
                                                          // vertex
  ...
  private static void printAllTopologicalSorts(HashMap<Integer, List<Integer>> graph,
    ...
    if (!sources.isEmpty()) {
      for (Integer vertex : sources) {
        ...
        List<Integer> children = graph.get(vertex); // get the node's children to decrement their
                                                    // in-degrees
        ...
        // backtrack, remove the vertex from the sorted order and put all of its children back to
        // consider
  ...
  public static void main(String[] args) {
    ...
    AllTaskSchedulingOrders.printOrders(6, new int[][] {new int[] {2, 5}, new int[] {0, 5},
